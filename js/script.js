...
document.getElementById("startFolderButton").addEventListener("click",function(e){
    const {dialog} = require('electron').remote;
    dialog.showOpenDialog({
        properties: ['openDirectory']
    },function(path){
        if(path){
            document.getElementById("startFolder").innerHTML=path[0];
        }else {
...
document.getElementById("endFolderButton").addEventListener("click",function(e){
    const {dialog} = require('electron').remote;
    dialog.showOpenDialog({
        properties: ['openDirectory']
    },function(path){
        if(path){
            document.getElementById("endFolder").innerHTML=path[0];
        }else {
            console.log("No path selected");
        }
    });
